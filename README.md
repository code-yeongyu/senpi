...
Other builtins such as `permission-system`, `prompt-preset`, `anthropic-bash`, `service-tier`, `tool-pair-guard`, `compaction`, `history-search`, and `session-observer` are senpi-owned builtin extensions, not installable sibling packages.
...
| 3 | [`prompt-preset`](packages/coding-agent/src/core/extensions/builtin/prompt-preset/) | Per-model system prompt presets (gpt-5.x, claude-opus-4-{5,6,7}, kimi-k2-{6,7}) layered on top of the dynamic prompt. Shared codex-style file-operations tuning. | [AGENTS.md](packages/coding-agent/src/core/extensions/builtin/prompt-preset/AGENTS.md) · [changes.md](packages/coding-agent/src/core/extensions/builtin/prompt-preset/changes.md) |
...
| 15 | [`websearch`](packages/coding-agent/src/core/extensions/builtin/websearch/) *(vendored)* | Provider-backed `web_search` tool + `/websearch` status command; providers include exa/tavily/brave/serper/google-cse/z-ai/perplexity/xai/openai/anthropic and **kimi** (Kimi-for-Coding search). Auto-routes to a native entry (incl. kimi-coding models) using the model key. Synced from [`code-yeongyu/pi-websearch`](https://github.com/code-yeongyu/pi-websearch). | — |
| 16 | [`webfetch`](packages/coding-agent/src/core/extensions/builtin/webfetch/) *(vendored)* | `webfetch` tool — URL content as markdown / text / HTML, bounded time + size; toggled via `PI_WEBFETCH`. Synced from [`code-yeongyu/pi-webfetch`](https://github.com/code-yeongyu/pi-webfetch). | — |
| 17 | [`nested-agents-md`](packages/coding-agent/src/core/extensions/builtin/nested-agents-md/) *(vendored)* | Auto-injects nearby `AGENTS.md` files when the agent reads from a nested directory; `/nested-agents` toggle. Synced from [`code-yeongyu/pi-nested-agents-md`](https://github.com/code-yeongyu/pi-nested-agents-md). | — |
| 18 | [`rules`](packages/coding-agent/src/core/extensions/builtin/rules/) *(vendored)* | Auto-discovers rule files (`.sisyphus/rules`, `.claude/rules`, `.cursor/rules`, `.github/instructions`, `AGENTS.md`, `CLAUDE.md`); `/rules` + `/reload-rules`. Synced from [`code-yeongyu/pi-rules`](https://github.com/code-yeongyu/pi-rules). | — |
| 19 | [`goal`](packages/coding-agent/src/core/extensions/builtin/goal/) *(vendored)* | Budget-free persistent goal tracking — `create_goal` / `update_goal` / `get_goal` + `/goal`, per-thread objective with continuation prompts. Synced from [`code-yeongyu/pi-goal`](https://github.com/code-yeongyu/pi-goal). | [AGENTS.md](packages/coding-agent/src/core/extensions/builtin/goal/AGENTS.md) · [changes.md](packages/coding-agent/src/core/extensions/builtin/goal/changes.md) |
