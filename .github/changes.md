...
- `.github/workflows/ci.yml`: split the single build-check-test job into parallel jobs - a static `check` job, a three-shard `test-coding-agent` vitest matrix, `test-workspaces` for script tests plus every workspace except coding-agent, a `check-and-test` fan-in gate that preserves the required "Check and test" status context, a three-OS `terminal-cross-os` job for the PTY package and terminal/shell suites (including native file-monitor fallback, secure worker failure/concurrency paths, and reload coverage), and a three-OS `inspector-handoff` job; Node 22 moved to 24, checkout/setup-node pins updated, apt made noninteractive, and per-job timeouts and step summaries added.
...
- The parallel CI split keeps the required status context stable while cutting wall time on the sharded coding-agent suite. Running file-monitor worker and lifecycle regressions in the existing terminal matrix covers the platform-specific `fs.watch`, child-process, path identity, and executable-routing branches on Ubuntu, macOS, and Windows. The read-only issue-analysis rewrite treats untrusted issue text as data, denies mutation, and strips secrets before any artifact leaves the runner.
