...
const CHATGPT_SUBSCRIPTION_BROWSER_LOGIN_METHOD = "browser";
...
async function startChatGptSubscriptionDeviceAuth(signal: AbortSignal): Promise<DeviceAuthInfo> {
...
async function pollChatGptSubscriptionDeviceAuth(
...
async function loginChatGptSubscriptionDeviceCode(interaction: ProviderAuthInteraction): Promise<OAuthCredential> {
	const device = await startChatGptSubscriptionDeviceAuth(interaction.signal);
	interaction.notify({
		type: "device_code",
	...
	const code = await pollChatGptSubscriptionDeviceAuth(device, interaction.signal);
...
async function loginChatGptSubscription(interaction: ProviderAuthInteraction): Promise<OAuthCredential> {
...
export const chatgptSubscriptionOAuth: OAuthAuth = {
	...
	async login(interaction) {
		const method = await interaction.prompt({
			...
			options: [
				{ id: CHATGPT_SUBSCRIPTION_BROWSER_LOGIN_METHOD, label: "Browser login (default)" },
				{ id: CHATGPT_SUBSCRIPTION_DEVICE_CODE_LOGIN_METHOD, label: "Device code login (headless)" },
			],
		});

		if (method === CHATGPT_SUBSCRIPTION_DEVICE_CODE_LOGIN_METHOD) {
			return loginChatGptSubscriptionDeviceCode(interaction);
		}
		if (method !== CHATGPT_SUBSCRIPTION_BROWSER_LOGIN_METHOD) {
