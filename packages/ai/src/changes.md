...
- `packages/ai/src/cursor/conversation-context-limit.ts`: a conversation-scoped record of the context limit Cursor reports for the live conversation, with `recordCursorConversationContextLimit(sessionId, conversationId, maxTokens)`, `getCursorConversationContextLimit(sessionId, conversationId?)` and `forgetCursorConversationContextLimit(sessionId?)`. Each session keeps the conversation that most recently reported, with the server-reported limit alongside it, and the session id scopes teardown. A reported limit of `0` is never recorded, and a checkpoint for a different conversation replaces the entry outright, so the new conversation can never inherit the previous one's value. Cursor can rotate a conversation's wire id without changing the base conversation id admission names, so the module also keeps a per-session base-to-wire map through `setCursorActiveConversationWire(sessionId, baseConversationId, wireConversationId)`: the reader resolves a published base identity to the wire that is actually live and returns that wire's own record only, so the rotation keeps the limit the live wire reported reachable while the rotated wire contributes nothing until its own checkpoint reports one.
