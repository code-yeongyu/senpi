...
- `packages/ai/src/auth/oauth/chatgpt-subscription.ts` and `packages/ai/src/utils/chatgpt-subscription-auth.ts`: ChatGPT logins record the signed-in person from the access token as the identity - `chatgpt_account_user_id` (or `chatgpt_user_id` scoped to the workspace when absent) with the profile email. The stored `accountId` (`chatgpt_account_id`) names the workspace, which every member of a Team workspace shares, so it is not used for matching.
