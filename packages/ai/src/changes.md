...
- `packages/ai/src/cursor/conversation-context-limit.ts`: a conversation-scoped record of the context limit Cursor reports for the live conversation, with `recordCursorConversationContextLimit(sessionId, conversationId, maxTokens)`, `getCursorConversationContextLimit(sessionId, conversationId?)` and `forgetCursorConversationContextLimit(sessionId?)`. Each session keeps the conversation that most recently reported, with the server-reported limit alongside it, and the session id scopes teardown. A checkpoint that carries no token details leaves the conversation's existing record in place, an explicit non-positive report drops it (so admission falls back to the bootstrap byte cap), and a checkpoint for a different conversation replaces the active entry outright, so the new conversation can never inherit the previous one's value. Cursor can rotate a conversation's wire id without changing the base conversation id admission names, so the module also keeps a per-session base-to-wire map through `setCursorActiveConversationWire(sessionId, baseConversationId, wireConversationId)`: the reader resolves a published base identity to the wire that is actually live and returns that wire's own record only, so the rotation keeps the limit the live wire reported reachable while the rotated wire contributes nothing until its own checkpoint reports one.
- `packages/ai/src/api/cursor-agent.ts`: the existing `onConversationCheckpoint` callback now records `checkpoint.tokenDetails.maxTokens` against the live wire `conversationId` (the session is kept only as the teardown scope), and the existing `registerSessionResourceCleanup` hook forgets it on session teardown alongside the conversation caches. Every attempt also publishes its `(baseConversationId, conversationId)` pair through `setCursorActiveConversationWire`, including the attempt after an in-call wire-id rotation, so the limit a checkpoint records on the rotated wire resolves for the base identity admission passes.
...
- Follow-up fix from review: `recordCursorConversationContextLimit` now takes `maxTokens: number | undefined` and stops collapsing the two ways a checkpoint can carry no ceiling. `undefined` - the checkpoint had no `tokenDetails` at all - preserves the conversation's existing record, while the server's explicit `0` deletes it, so a conversation that reported 200000 and later reports zero admits at exactly 50000 bytes instead of keeping an 800000-byte budget. `api/cursor-agent.ts` passes `checkpoint.tokenDetails?.maxTokens` instead of `?? 0`, which is what made the distinction representable at the call site. The unit test that asserted the old preservation semantics on a literal `0` while its name claimed the checkpoint "omits" the limit was corrected: the omission case now passes `undefined` and asserts the limit survives, and new explicit-positive-then-explicit-zero cases assert the record is dropped for both the direct and the published-wire lookup.
...
- Cursor rotates a conversation's wire id in-call while the base id admission names (`options.conversationId ?? options.sessionId`) stays put: keying the record to the rotated wire alone made the limit unreachable, so admission silently fell back to the 50000-byte bootstrap cap and resumed deleting whole turns. The provider therefore publishes the live wire for the base identity and the reader resolves through it. Limits stay attached to the wire that reported them, so the isolation rule is intact: a rotated or brand-new wire starts unknown and only its own positive checkpoint can raise its budget.
...
- A checkpoint with no `tokenDetails` and a checkpoint whose `maxTokens` is explicitly `0` are different reports, and the first revision merged them with `?? 0`: a conversation that had reported 200000 could never fall back, so an explicit zero from the server kept admission at 800000 bytes where the 50000-byte bootstrap cap was required.
...
- LOW: the `onConversationCheckpoint` closure, the `setCursorActiveConversationWire` publish call, and the `registerSessionResourceCleanup` call in `api/cursor-agent.ts`; the export list in `index.ts`.
