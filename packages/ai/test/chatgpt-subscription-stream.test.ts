...
describe("openai-codex streaming", () => {
	...
	it("forwards auto transport from streamSimple options and uses cached websocket context", async () => {
		...
		expect(getChatGptSubscriptionWebSocketDebugStats("session-auto")).toMatchObject({
	...
	it("scopes cached websockets to the authenticated account", async () => {
		...
		expect(global.fetch).not.toHaveBeenCalled();
		expect(getChatGptSubscriptionWebSocketDebugStats("shared-session")).toMatchObject({
	...
	it("closes one-shot websockets when cacheRetention is none", async () => {
		...
		expect(getChatGptSubscriptionWebSocketDebugStats("one-off-summary")).toBeUndefined();
		expect(global.fetch).not.toHaveBeenCalled();
	});
	...
	it("falls back to SSE when websocket connect does not open before the connect timeout", async () => {
		...
		expect(result.content.find((content) => content.type === "text")?.text).toBe("Hello");
		expect(fetchMock).toHaveBeenCalledTimes(1);
		expect(getChatGptSubscriptionWebSocketDebugStats("ws-connect-timeout")).toMatchObject({
			websocketFailures: 1,
			sseFallbacks: 1,
	...
	it("falls back to SSE when a websocket is idle before the first event", async () => {
		...
		expect(result.content.find((content) => content.type === "text")?.text).toBe("Hello");
		expect(fetchMock).toHaveBeenCalledTimes(1);
		expect(getChatGptSubscriptionWebSocketDebugStats("ws-idle-before-start")).toMatchObject({
			websocketFailures: 1,
			sseFallbacks: 1,
			websocketFallbackActive: true,
		});
	...
	it("sends only response input deltas in websocket-cached mode", async () => {
		...
		expect(getChatGptSubscriptionWebSocketDebugStats("session-1")).toMatchObject({
			requests: 2,
	...
	it("retries websocket continuations when Codex reports a stale previous response anchor", async () => {
		const token = mockToken();
		const sentBodies: unknown[] = [];
		...
		class MockWebSocket {
			...
			send(data: string): void {
				...
				const requestIndex = sentBodies.length;

				if (requestIndex === 1) {
					this.emitResponse("resp_1", "msg_1", "First answer");
					return;
				}
				...
				if (requestIndex === 3) {
					...
					this.emitResponse("resp_3", "msg_3", "Second answer");
					return;
				}
			...
			private emitResponse(responseId: string, messageId: string, text: string): void {
				const events = [
					{ type: "response.created", response: { id: responseId } },
					{
						type: "response.output_item.added",
		...
		expect(second.content.find((content) => content.type === "text")?.text).toBe("Second answer");
		expect(sentBodies).toHaveLength(3);
		const retryBody = sentBodies[2] as { input?: unknown[]; previous_response_id?: string };
		expect(retryBody.previous_response_id).toBeUndefined();
		expect(JSON.stringify(retryBody.input)).toContain("First question");
		expect(JSON.stringify(retryBody.input)).toContain("Second question");
		expect(global.fetch).not.toHaveBeenCalled();
	});
	...
	it("zstd-compresses SSE request bodies", async () => {
		...
		let capturedBody: ArrayBuffer | Uint8Array | string | undefined;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			capturedBody = init?.body as ArrayBuffer | Uint8Array | string | undefined;
			return new Response(
				new ReadableStream<Uint8Array>({
					start(controller) {
						controller.enqueue(encoder.encode(sse));
		...
		const decodedBody =
			capturedBody instanceof Uint8Array ? capturedBody : new Uint8Array(capturedBody as ArrayBuffer);
		const decoded = JSON.parse(Buffer.from(zstdDecompressSync(decodedBody)).toString("utf8")) as {
