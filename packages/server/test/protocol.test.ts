...
describe("pi-ai protocol bridge", () => {
	...
	test("keeps fork-only AI content outside the upstream protocol schema", () => {
		const videoModel = { ...model, input: ["text", "image", "video"] } as unknown as Model<Api>;
		expect(toProtocolModelMetadata(videoModel, true).input).toEqual(["text", "image"]);

		const providerNativeMessage = {
			role: "assistant",
			content: [{ type: "providerNative", subtype: "server_tool_use", raw: { id: "native-1" } }],
			api: "test-api",
			provider: "test-provider",
			model: "model-1",
			usage: {
		...
		expect(toProtocolAssistantMessage(providerNativeMessage, { id: "message-native" }).content).toEqual([]);
	});
