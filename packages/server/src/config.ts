...
const ENV_SERVER_DIR = "SENPI_SERVER_DIR";
...
export function getServerDir(): string {
	const envDir = process.env[ENV_SERVER_DIR] || process.env[ENV_ORCHESTRATOR_DIR];
	if (envDir) {
		return envDir;
	}
...
export const getOrchestratorDir = getServerDir;
...
export function getSocketPath(): string {
	return join(getServerDir(), "orchestrator.sock");
}
