...
Experimental local server for the new durable Session and Agent Harness interfaces. The package is `@code-yeongyu/senpi-server`, stays private (not published to npm), and is imported inside the workspace as `@earendil-works/pi-server`; the CLI it serves is `senpi`.
