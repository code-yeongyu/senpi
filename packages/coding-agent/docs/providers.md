...
These providers hand the current conversation to the native agent for one turn. The native agent owns its internal tools; Senpi does not translate those tool calls into Senpi host tools. Kimi mediates ACP tool requests through Senpi's permission policy and cannot persist an always-allow choice. Codex's SDK does not expose approval callbacks, so Codex instead runs with an isolated temporary home, workspace-write sandbox, untrusted approval policy, and network access disabled. Grok 1.0.0 executes its native tools but does not expose a reliable blocking permission callback to ACP clients, so Senpi permission rules do not gate Grok's internal tool calls; use Grok's own permission configuration and sandbox when that boundary is required. Child processes receive a minimal runtime environment plus only their own provider credential variables; `HOME` and XDG paths remain available because the native CLIs store their own credentials and configuration there. Native CLI credentials are not copied into Senpi's `auth.json`.
