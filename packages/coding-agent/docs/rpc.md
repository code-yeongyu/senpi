...
keeps its successful authenticated readiness connection attached through the remaining ownership and state work, then
releases it as the call returns. The caller therefore receives one complete idle window in which to attach; slow
post-probe filesystem work cannot consume that window before the handoff.
