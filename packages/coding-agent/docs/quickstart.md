...
Use `/model` or Ctrl+L to choose a model. Shift+Tab keeps cycling thinking levels, while Alt+A (`app.approval.cycle`) cycles the session's approval mode through workspace, read-only, and ask. Use Ctrl+P / Shift+Ctrl+P to cycle through favorite models.
