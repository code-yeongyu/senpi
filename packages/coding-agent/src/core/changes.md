...
- `packages/coding-agent/src/core/agent-session.ts` gains `resolveCursorAdmissionMaxBytes(reportedContextTokens)` and uses it at the Cursor `transformContext` call site instead of passing `CURSOR_TOOL_RESULT_MAX_BYTES` directly, so the aggregate admission budget is derived from the server-reported conversation context limit rather than the hardcoded 50,000-byte cap. The reported limit is read with the conversation-scoped `getCursorConversationContextLimit(sessionId, sessionId)`, where the session id mirrors the provider's own `options.conversationId ?? options.sessionId` derivation (this host does not override `conversationId`, so the conversation a request runs on is the agent's session id). That identity is stable across Cursor's in-call wire-id rotation: the provider publishes the live wire for it, so the rotated conversation's own reported limit stays reachable, while a conversation with neither a published wire nor a report bootstraps at the legacy cap. Tokens are converted using the repo-wide 4-characters-per-token estimate, and the result never drops below `CURSOR_TOOL_RESULT_MAX_BYTES`.
- `truncateToolResultBodies`' aggregate turn-dropping pass no longer compares only against the full-wire measurement. When the budget exceeds the fixed legacy cap - which can only happen with a server-reported limit - the drop gate uses `measureCursorModelInputSerializedBytes` (the `rootPromptMessagesJson` blobs Cursor builds the model prompt from), so the `buildConversationTurns` display copies cannot trigger whole-turn deletion. The fixed 50,000-byte bootstrap cap keeps its existing wire-level gate, and the per-result character cap and tool-body blanking pass are unchanged, so issue #1043's envelope bounds still hold.
- Follow-up fix from review: an explicit zero checkpoint invalidates the conversation's recorded limit while a checkpoint that carries no token details leaves it in place. `resolveCursorAdmissionMaxBytes` returns exactly `CURSOR_TOOL_RESULT_MAX_BYTES` (50,000) for an absent, non-finite, or non-positive report, so the bootstrap is unchanged - the first checkpoint of every conversation still reports `maxTokens` 0 - and a conversation that reported 200,000 drops back to exactly 50,000 after the server reports an explicit zero instead of keeping an 800,000-byte budget.
- `packages/coding-agent/test/suite/regressions/1603-cursor-history-budget.test.ts`: a tool-free Cursor conversation whose first turn alone exceeds the legacy 50,000-byte cap keeps that turn; the reviewer's 410,000-character counter-case (model input inside the 800,000-byte reported budget, full wire outside it) keeps its sentinel and drops no turn; a replacement conversation under a session whose other conversation reported 200,000 admits at exactly 50,000, and the session's own conversation scales to 800,000 only after it reports; after a wire-id rotation, the rotated conversation's 200,000-token report keeps all 3 messages and the sentinel through the installed transform instead of collapsing at the 50,000-byte fallback; the explicit-zero cases assert the budget drops back to exactly 50,000 both directly and through the installed transform.
...
- An explicit `maxTokens: 0` is the server reporting no established ceiling yet (the bootstrap report), which must fall back to exactly the 50,000-byte cap - not a checkpoint that omitted its token details: merging the two with `?? 0` meant a conversation which had reported 200,000 kept that report past its later zero checkpoint, so admission sized the next request at 800,000 bytes instead.
...
- LOW: the Cursor admission constants and the `transformContext` closure in `packages/coding-agent/src/core/agent-session.ts`.
