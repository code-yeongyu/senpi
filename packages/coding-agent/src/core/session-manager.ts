...
import { addUsageToTotals } from "./usage-totals.ts";
...
export class SessionManager {
	...
	// incrementally on usage-bearing entry append and rebuilt from scratch in
		...
		const usage =
			entry.type === "message" && (entry.message.role === "assistant" || entry.message.role === "toolResult")
				...
				: entry.type === "branch_summary" || entry.type === "compaction"
					...
					: undefined;
		if (!usage) return;
		addUsageToTotals(this.usageTotals, usage);
