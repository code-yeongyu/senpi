...
- Added `grok-4.5.ts`: sibling-scale `tuningSection` for Grok 4.5. Live content is the winner of a controlled two-preset Mario browser-game benchmark: act once context is sufficient; do not re-derive settled facts or survey discarded options; audit claims against current-session tool evidence; pause only for destructive/irreversible work, real scope change, or user-only input; never end on a plan/promise when the work can be executed; keep final summaries complete and concise; continue through context pressure instead of stopping early.
- Benchmark evidence is under `local-ignore/qa-evidence/20260717-grok45-mario-benchmark/`: identical `cliproxy/grok-4.5` model/task/settings, a sanitized run manifest, independent static QA, real browser keyboard play, source inspection, and the persisted blind scorecard/mapping. The retained preset scored 93.5 vs. 86.9; the decisive production difference was complete focus-loss key clearing and safer jump/input lifecycle in its generated game.
- Does **not** assert any model identity/lineage claim — unverified identity/lineage wording is omitted from live tuning.
- Omits full-document scaffolding already owned by the shared dynamic prompt (no `PRIMARY ATTENTION ANCHOR` / `MANDATORY RESPONSE PROTOCOL` / `SILENT PRE-SEND CHECK` section dumps).
- `presets.ts`: `hasGrok45Signal` / `isGrok45Model` match any Grok 4.5 id shape (`grok-4.5`, `grok45`, `xai:grok-4.5`, path/prefix ids, trailing tags) without catching `grok-4.3` / `grok-4.20-*` / `grok-3`.
- `settings.ts`: `"grok-4.5"` joins `PromptPresetName` / `VALID_PRESETS`.
- `AGENTS.md`: lists the Grok preset file.
- `test/suite/prompt-presets-grok-4-5.test.ts`: id resolution, negative neighbors, settings force, catalog coverage, execution/evidence contract pins, a 900–1800 character tuning-size guard, and negative pins against the rejected Fable/lineage and alternate intent-gate framing.

### Why
- Grok 4.5 needs family-specific execution discipline without a full `corePrompt` rewrite. In the production-style Mario benchmark, evidence-grounded continuation and claim-auditing produced the stronger keyboard lifecycle and passed every hard gate; the Grok-specific identity/low-reasoning alternative did not. Sibling non-corePrompt presets stay ~1–2kB of model-family tuning; raw multi-section contracts and stacked fake “editions” are both wrong for an unreleased surface.
