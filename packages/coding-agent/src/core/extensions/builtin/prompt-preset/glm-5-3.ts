...
export interface Glm53Rule { readonly id: Glm53RuleId; readonly concern: Glm53Concern; readonly directive: string; }
export const GLM_53_RULES: readonly Glm53Rule[] = [{ id: "load-matching-skills", concern: "skill-utilization", directive: "Before the first non-discovery action, compare the task with every visible skill description. For each loose match, read that skill's listed `SKILL.md` and follow it; when matching work is delegated, include the skill name in `load_skills`. Proceed without a skill only after this scan finds no match - recognizing a match without loading it does not complete the gate." }];
