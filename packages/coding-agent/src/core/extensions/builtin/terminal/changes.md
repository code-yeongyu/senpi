...
- Each directory worker shares one absolute `fs.watch(process.cwd())` wake hint and one one-second anchored stat recheck across its targets. Windows disables only the optional watch hint because libuv aborts when a watched directory is renamed; anchored polling remains active on every platform, watch registration/runtime failures leave it active, and every deadline performs one final reconciliation before reporting a timeout.
