...
- Each directory worker shares one absolute `fs.watch(process.cwd())` wake hint and one one-second anchored stat recheck across its targets. The absolute Windows path avoids libuv's relative-directory assertion, watch registration/runtime failures leave polling active, and every deadline performs one final reconciliation before reporting a timeout.
