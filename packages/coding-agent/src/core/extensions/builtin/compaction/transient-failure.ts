 ...
 * loud). Watchdog trips — a stalled stream, a stream that outlived its
 * wall-clock budget, or a compaction that outlived its total budget — are
 * infrastructure-slowness outcomes and always degrade.
...
export function isTransientSummarizationFailure(error: unknown, message: string): boolean {
	...
	if (error instanceof SummarizationTotalBudgetError) return true;
