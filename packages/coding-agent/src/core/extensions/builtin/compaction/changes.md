...
- Required threshold/overflow recovery may synthesize one local checkpoint after a summarization watchdog or a transient `SummaryRequestError` carrying the structured `upstream-stream-truncated` failure kind, without issuing another provider request. Pre-prompt recovery uses the same path only when current usage has reached `contextWindow - reserveTokens`; optional pre-prompt compaction below that boundary remains fail-closed. Generic thrown text is never fallback authorization, even when it contains truncation-like markers.
- The extension watchdog now covers final `responseStream.result()` settlement as well as stream acquisition and iteration, so an ended provider iterator cannot leave required compaction pending indefinitely.
- A linked outer 120-second deadline now covers the full extension compaction operation from credential resolution onward. Pre-stream provider preparation can no longer bypass the typed watchdog path; caller abort still resolves as cancellation rather than timeout.
- Recovery is accepted only with a real non-empty retained boundary whose fully reconstructed content fits `contextWindow - reserveTokens`, including the exact cap boundary. Fit is recomputed from retained message content because assistant usage recorded before compaction describes the discarded context and can otherwise falsely reject a small suffix. An absent or genuinely unfit suffix cancels without appending a compaction entry or dropping the latest request.
