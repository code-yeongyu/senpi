...
export function buildAnthropicMessagesSearchRequest(ctx: BuildContext, defaultModel: string): BuiltSearchRequest {
	const { config, request, allowedDomains, blockedDomains } = ctx;
...
export function normalizeAnthropicMessagesSearchPayload(data: JsonObject): SearchResultItem[] {
...
export const anthropicProvider: ProviderModule = {
	buildRequest(ctx: BuildContext): BuiltSearchRequest {
		return buildAnthropicMessagesSearchRequest(ctx, "claude-sonnet-4-5-20250929");
	},
