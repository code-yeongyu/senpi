...
- The extension directory moved `packages/coding-agent/src/core/extensions/builtin/claude-sdk-oauth/` -> `.../anthropic-subscription/` (`git mv`, history preserved), and the identifier family renamed to the new spelling: `CLAUDE_SDK_OAUTH_*` -> `ANTHROPIC_SUBSCRIPTION_*`, `ClaudeSdkOauth*` -> `AnthropicSubscription*`, `claudeSdkOauth*` -> `anthropicSubscription*` (e.g. `ANTHROPIC_SUBSCRIPTION_PROVIDER_ID`, `loadAnthropicSubscriptionProviderSettings`, `buildAnthropicSubscriptionQueryOptions`). The matching test cluster (`test/claude-sdk-oauth-*.test.ts` -> `test/anthropic-subscription-*.test.ts`), helpers and `.agents/skills/senpi-qa/scripts/claude-sdk-oauth-*` were renamed likewise, and every importer updated. Behaviour-free: no string literal changed.

### Why

Completion of the provider rename (senpi#1989): after the provider id became `anthropic-subscription`, the internal spellings still named an implementation detail that no longer exists, so every grep for the provider surfaced internals that name nothing real. Deliberately its own commit so review can separate the mechanical rename from the semantic changes.

### Why an extension could not handle it

This IS the extension: the directory name, its import graph and its symbol names are the thing renamed.

### Expected merge conflict zones

- Every file in this directory and every importer of `extensions/builtin/anthropic-subscription/*`: `src/core/agent-session.ts`, `src/core/credential-accounts.ts`, `src/modes/rpc/connection-handler.ts`, `src/modes/app-server/server/account.ts`, `src/modes/app-server/server/server-core.ts`, `src/core/extensions/builtin/index.ts`, `.../builtin/compaction/lane-policy.ts`, `.../builtin/gpt-account.ts`, `.../builtin/cursor-cli-oauth/account-command.ts`.
- The renamed test cluster under `packages/coding-agent/test/` and the `.agents/skills/senpi-qa/scripts/` probes; PRs touching the old paths must rebase onto this rename.

Frozen on purpose (byte-identical values): `ANTHROPIC_SUBSCRIPTION_COMPACT_ENTRY_TYPE` is still `"claude-sdk-oauth-compact"`, the boundary schema still `senpi.claude-sdk-oauth.compact-boundary.v1`, the sidecar still `.claude-sdk-oauth-binding.json`, the tool-watch type still `claude-sdk-oauth-tool-watch`, the managed sentinel still `claude-sdk-oauth-managed`, every `claude_sdk_oauth_*` diagnostic id, every `SENPI_CLAUDE_SDK_OAUTH_*` env-var name, `CLAUDE_CODE_OAUTH_TOKEN*`, the wire api id `CLAUDE_SDK_OAUTH_API_ID` (`"claude-sdk-oauth"`), and the legacy settings-read property `claudeSdkOauthProvider` (the two-release compatibility window).
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/index.ts`: `readStoredCredential` reads auth.json through the canonical key and then the legacy spelling.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/tool-watch.ts`: the provider comparison normalizes `ctx.model?.provider` before comparing.
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/index.ts` `readStoredCredential`.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/tool-watch.ts` the constant block and the `tool_execution_end` guard.

## 2026-09-22 - read the renamed provider settings block (senpi#1989)

### What changed

- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/settings.ts`: `loadAnthropicSubscriptionProviderSettings` now reads the canonical `anthropicSubscriptionProvider` block, falling back to the legacy `claudeSdkOauthProvider` key; the `SettingsWithAnthropicSubscriptionProvider` type carries both.
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/config-dir-credentials.ts` `resolveAccountsDirectory` and the accounts-dir name constants.

## 2026-09-22 - claude-sdk-oauth provider id renamed to anthropic-subscription across the extension (senpi#1989)

### What changed

- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-management.ts`: `ANTHROPIC_SUBSCRIPTION_PROVIDER_ID` value is `"anthropic-subscription"` (symbol name unchanged).
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-commit-boundary.ts`: `isResidentAssistant` compares `message.api === CLAUDE_SDK_OAUTH_API_ID` and `message.provider === ANTHROPIC_SUBSCRIPTION_PROVIDER_ID`. The old code compared BOTH fields to the provider-id constant, which only worked while api id === provider id; after the value rename it would always be false and the #7925 resident commit boundary would silently die.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/tool-watch.ts`: runtime `PROVIDER_ID` comparison moves to the new id; `TOOL_WATCH_CUSTOM_TYPE` (`claude-sdk-oauth-tool-watch`) is a persisted session custom-entry type and stays.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/guidance.ts`: rendered guidance (`/login anthropic-subscription`) and the override-prompt error use the new id/label.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/oauth-login.ts`: login label is `"Anthropic Subscription (Claude Pro/Max)"`; auth-check source follows.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-command.ts`: label constant and every notify/description string read "Anthropic Subscription".
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/affinity.ts`, `bounded-queue.ts`, `failover.ts`, `session-stream.ts`, `session-registry.ts`, `session-registry-pump.ts`, `session-reattach.ts`, `session-turn-claim.ts`: user-visible error/notify strings use the new label. `DEFAULT_AFFINITY_KEY` (`claude-sdk-oauth-default`) is FROZEN — it is the HRW hash-domain constant; renaming it silently remaps every unpinned session to a different account.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-observability.ts`: the close-cause regex accepts BOTH phrasings (`Claude SDK OAuth query|Anthropic Subscription query`) so classification survives the renamed pump messages.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/auth-lane.ts`: the no-managed-accounts error names the new lane and `/login anthropic-subscription`; `CLAUDE_CODE_OAUTH_TOKEN` unchanged.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/errors.ts`: doc comment names the new label.
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/api-id.ts`: new `CLAUDE_SDK_OAUTH_API_ID`, frozen at `"claude-sdk-oauth"`. That string is the wire identity sent to the provider and persisted in user data; it must not follow a provider-id rename.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/index.ts`: `registerProvider` still takes the provider id from `ANTHROPIC_SUBSCRIPTION_PROVIDER_ID`. `api` and `baseUrl` now come from `CLAUDE_SDK_OAUTH_API_ID`.
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-registry-wiring.ts`: the `model_select` handler no longer invalidates when the newly selected model belongs to another provider. It now uses the existing `keepBindingThenClose`, exactly like `thinking_level_select` and the in-provider `switchSessionModel` failure branch, so the live SDK session closes while the binding and its sidecar survive. The same file now also carries the ledger invalidation cause into process memory: `persistBindingInvalidation` records it as it appends the record, `session_start` re-reads it from the branch on a restart (clearing it for `new`), and the `message_end` marker retires it.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-binding.ts`: adds `invalidationReasonFromBranch`, which returns the reason of the newest binding ledger record when that record is an invalidation (a later marker retires it).
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-reattach.ts`: holds the pending invalidation reason per senpi session id (`rememberBindingInvalidation`, `bindingInvalidationReason`) next to the binding map it replaces.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-continuity.ts`: `ContinuityDecisionInput` accepts `invalidationReason`, and a `bootstrap`/`flatten` that would report the no-record default `registry_miss` reports the recorded cause instead (`model_selected`, `extensions_removed`, `assistant_rewritten` pass through the observation vocabulary; `compaction`, `tree_changed` and `fork` map to `tainted_compaction`, `branch_diverged` and `tainted_fork`). Classification is unchanged: no decision kind moves, and every other reason is left as decided.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/session-stream.ts`: passes the pending reason into the decision input.
...
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/accounts.ts`: `AccountSlot` carries optional `displayName`; immutable `name` remains the operational identity.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-management.ts`: explicitly projects safe display metadata alongside existing secret-free account descriptors.
- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-command.ts`: adds rename/clear-name and renders display labels for listings, the pinned-account line and the affinity pick. Post-login naming is delegated to the shared helper, which offers it only for a machine-generated slot ID, so the lane's own "Name for this account" prompt is never followed by a second name prompt. Existing slot creation names, sentinel envelope, refresh, failover, affinity and session continuity are unchanged.

### Why

- `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/accounts.ts`, `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-management.ts` and `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/account-command.ts`: readable labels must not change the account IDs responsible for Claude credentials, config roots and SDK session bindings.
...
- `test/anthropic-subscription-accounts.test.ts`: a pool holding a real account plus a generated `login-2` sentinel slot lists only the real one.
...
- `test/anthropic-subscription-reattach.test.ts`: cover both successful initialization
...
- LOW: `executable.ts` is fork-only; `availability.ts` grows an export above `createAmbientAuthStatusReader`. `executable-path-lookup.ts` is new. Tests: `test/claude-sdk-oauth-executable*.test.ts`, `test/anthropic-subscription-availability.test.ts`.
...
- `anthropic-subscription-bootstrap-classification.test.ts` covers both observations through the resident session-stream fake SDK boundary.
...
- Removed `decideFailoverContinuity`, `FailoverContinuityInput`, `FailoverLane`, and `test/claude-sdk-oauth-failover-continuity.test.ts`. Tests were flipped in `anthropic-subscription-restart-binding-drift.test.ts`, `anthropic-subscription-restored-security.test.ts`, and `anthropic-subscription-continuity-retry-checkpoint.test.ts`; added regression coverage in `suite/regressions/1432-anthropic-subscription-failover-reattach.test.ts`.
...
- Tests: `anthropic-subscription-restart-binding-drift.test.ts` (new) pins the restart contract and midnight stability with trailing appends; `anthropic-subscription-fingerprint.test.ts`, `anthropic-subscription-restored-security.test.ts`, `anthropic-subscription-continuity-decision.test.ts`, `anthropic-subscription-session-registry-wiring.test.ts` expectations moved from `options_changed` / `flatten` to the split reasons / `reattach`.
...
- `test/anthropic-subscription-session-registry.test.ts` covers the deterministic early-result ordering.
...
- `session-registry.ts`: `AnthropicSubscriptionSessionEntry.sdkSessionIdConfirmed` records whether the SDK acknowledged the entry's session id; entries created from a resume start confirmed, entries whose id `getOrCreate` minted locally start unconfirmed.
...
- LOW in `options.ts` around `buildAnthropicSubscriptionQueryOptions` tool selection and strict MCP configuration.
...
- `settings.ts`: `loadAnthropicSubscriptionProviderSettingsFromDisk` caches the `SettingsManager` instance keyed on (cwd, mtimeMs:size of the global and project settings.json). A cache hit skips `SettingsManager.create` and its two locked disk reads; environment overrides are re-parsed on every call so live env changes take effect immediately.
...
- `settings.ts` around `loadAnthropicSubscriptionProviderSettingsFromDisk`.
...
- `auth-lane.ts` classifies that error as non-retryable before account failover, `stream.ts` terminates non-resident
  ...
  message.
- `test/anthropic-subscription-refusal.test.ts` covers structured ambient and resident refusals plus the legacy assistant
...
- `session-reattach.ts`: `ContinuityBinding` gained an optional in-memory-only `unansweredTurnDigest`. It
  ...
  Tests: `test/anthropic-subscription-binding-store.test.ts` (round-trip rejects it).
...
- Tests: `test/suite/regressions/723-anthropic-subscription-timeout-abort-retry-continuity.test.ts`,
  `test/anthropic-subscription-continuity-decision.test.ts`, `test/anthropic-subscription-continuity-retry-checkpoint.test.ts`.
...
- The three async entry points that reach a synchronous SDK member now await the loader first:
  `stream.ts` (`streamAnthropicSubscription`, before `getSdkBoundary().query` and the resident-session lane),
...
- `index.ts`: the provider registration passes `fallbackEligible`, returning false only under the
  ...
  Tests: `test/suite/anthropic-subscription-fallback-eligibility.test.ts`.
...
- LOW in `settings.ts` (`AnthropicSubscriptionProviderSettings` field list, `parseProviderSettings` and
...
- LOW in `test/support/anthropic-subscription-provider.ts` (`composedProvider` gained a third `settings` parameter) and in the
...
- This entry is the canonical inventory for the repository-wide changes.md audit (`scripts/audit-changes-md.mjs`, pin
  ...
  none — every file under `packages/coding-agent/src/core/extensions/builtin/anthropic-subscription/` is fork-only (absent
...
- `anthropic-subscription-availability.test.ts` drives the two outcome cases through
...
- `index.ts` wires `readSettings` to `loadAnthropicSubscriptionProviderSettingsFromDisk(process.cwd())`.
- `options.ts` `buildAnthropicSubscriptionQueryOptions` aligns its `authLane` fallback to `ambient` so both lane-resolution sites agree (the previous `?? "oauth-slots"` was a dead default contradicted by `managedPool`).
...
- Added `test/suite/regressions/790-anthropic-subscription-empty-user-continuity.test.ts` covering the transient empty
...
- Expected merge conflict zones: LOW in `session-commit-boundary.ts`; LOW in
  `test/suite/regressions/691-anthropic-subscription-thinking-timing.test.ts`.
...
- Merge-conflict risk: low. The only overlap surface is the settings/env-resolution block in `buildAnthropicSubscriptionQueryOptions`, which the concurrent `stream.ts` / `auth-lane.ts` work also touches.
...
- Why an extension could not handle it: the `append` list is assembled inside `buildAnthropicSubscriptionQueryOptions`, which no extension hook can reach.
...
- Merge-conflict risk: low. Expected conflict zones are the `append` array literal in `buildAnthropicSubscriptionQueryOptions` and the extractor cluster next to `extractSkillsAppend`.
