...
type ManualApprovalMode = Exclude<PermissionPresetName, "full-access" | "auto">;
type ApprovalMode = "auto" | ManualApprovalMode;
...
export class ApprovalModeCycle {
	...
	next(): ApprovalMode {
		...
		this.current = mode;
		this.service.setSessionPreset(rulesForPreset(mode === "auto" ? "ask" : mode));
