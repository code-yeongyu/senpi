...
- `packages/coding-agent/test/suite/regressions/1603-cursor-history-budget.test.ts`: a tool-free Cursor conversation whose first turn alone exceeds the legacy 50,000-byte cap keeps that turn; the reviewer's 410,000-character counter-case (model input inside the 800,000-byte reported budget, full wire outside it) keeps its sentinel and drops no turn; a replacement conversation under a session whose other conversation reported 200,000 admits at exactly 50,000, and the session's own conversation scales to 800,000 only after it reports; after a wire-id rotation, the rotated conversation's 200,000-token report keeps all 3 messages and the sentinel through the installed transform instead of collapsing at the 50,000-byte fallback.
- Follow-up fix from review: an explicit zero checkpoint now invalidates the conversation's recorded limit. The admission budget is asserted to drop back to exactly 50,000 bytes after 200,000 is followed by an explicit zero, both directly and through the installed transform, where the reviewer's 410,000-character counter-case collapses again - correcting the earlier behaviour that kept 200,000 (and admitted at 800,000 bytes) because an explicit zero was indistinguishable from a checkpoint that carried no token details.
