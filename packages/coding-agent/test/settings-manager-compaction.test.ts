...
/**
 * The fork's `getCompactionSettings()` resolves the whole compaction policy (speculation,
 * restoration, grace band, summarization model, ...); these cases pin the token budget
 * fields upstream's per-model overrides resolve, so they compare that projection.
 */
function tokenSettings(manager: SettingsManager, forModel?: { provider: string; id: string }) {
	const { enabled, reserveTokens, keepRecentTokens } = manager.getCompactionSettings(forModel);
...
describe("compaction model overrides", () => {
	it("uses defaults without compaction settings", () => {
		...
		expect(tokenSettings(manager)).toEqual(defaults);
		expect(tokenSettings(manager, model)).toEqual(defaults);
	});
	...
	it("resolves each field independently and keeps individual getters consistent", () => {
		...
		expect(tokenSettings(manager, model)).toEqual({
			enabled: true,
		...
		expect(tokenSettings(manager)).toEqual({ enabled: true, reserveTokens: 8192, keepRecentTokens: 10000 });
	...
	it("falls back to built-in defaults for missing fields", () => {
		...
		expect(tokenSettings(manager, model)).toEqual({ ...defaults, keepRecentTokens: 1024 });
	});
	...
	it("matches exact provider/model IDs, including IDs containing slashes", () => {
		...
		for (const other of [
			...
			expect(tokenSettings(manager, other)).toEqual(defaults);
		}
	});
	...
	it("merges project model overrides per field before resolving fallbacks", async () => {
		...
		expect(tokenSettings(manager, { provider: "provider", id: "other" })).toEqual({
			enabled: true,
			reserveTokens: 1024,
	...
	it("keeps enabled global and preserves overrides when saving the toggle", async () => {
		...
		await manager.flush();
		await manager.reload();
		expect(tokenSettings(manager, model)).toEqual({ ...defaults, enabled: false, reserveTokens: 400000 });
	});
	...
	describe.each(["reserveTokens", "keepRecentTokens"] as const)("model override %s", (field) => {
		it.each([null, -1, 1.5, "400000", true, {}, [], Number.MAX_SAFE_INTEGER + 1])(
			...
			(value) => {
				...
				expect(tokenSettings(manager)).toEqual(defaults);
				expect(tokenSettings(manager, { provider: "other", id: model.id })).toEqual(defaults);
			},
		);
	...
	it("accepts zero in ordinary settings and model overrides", () => {
		...
		expect(tokenSettings(manager, model)).toEqual({ enabled: true, reserveTokens: 0, keepRecentTokens: 0 });
