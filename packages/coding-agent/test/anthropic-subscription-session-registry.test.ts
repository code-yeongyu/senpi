...
import {
	annotateBranchInfo,
	annotateTainted,
} from "../src/core/extensions/builtin/anthropic-subscription/session-entry-annotations.ts";
import {
...
import {
	...
	submitSessionTurn,
} from "../src/core/extensions/builtin/anthropic-subscription/session-registry-pump.ts";
import { transitionSessionState } from "../src/core/extensions/builtin/anthropic-subscription/session-registry-state.ts";
...
describe("Claude SDK OAuth session registry", () => {
	...
	it("throws for illegal state transitions", () => {
		overrideSessionRegistryBoundary({ queryFactory: () => fakeQuery() });
		const entry = new AnthropicSubscriptionSessionRegistry().getOrCreate(input("session-a"));
