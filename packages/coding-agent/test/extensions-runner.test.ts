...
describe("ExtensionRunner", () => {
	...
	describe("command collection", () => {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			...
			expect(
				runner.createContext(commands[0]!.sourceInfo.path).resolveOwnCommandInvocationName?.("shared-cmd"),
			).toBe("shared-cmd:1");
			expect(
				runner.createContext(commands[1]!.sourceInfo.path).resolveOwnCommandInvocationName?.("shared-cmd"),
			).toBe("shared-cmd:2");
		});
	});
