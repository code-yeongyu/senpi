...
import { forgetBinding, getBinding } from "../src/core/extensions/builtin/anthropic-subscription/session-reattach.ts";
import {
	closeSession,
	getOrCreateSession,
	getSession,
...
describe("claude-sdk-oauth model and thinking switches", () => {
	it("switches models on the live query without starting a new session", async () => {
		...
		await emit(handlers, "model_select", {
			type: "model_select",
			model: { id: "claude-sonnet-5", provider: ANTHROPIC_SUBSCRIPTION_PROVIDER_ID },
			previousModel: { id: "claude-opus-4-5", provider: ANTHROPIC_SUBSCRIPTION_PROVIDER_ID },
		});
