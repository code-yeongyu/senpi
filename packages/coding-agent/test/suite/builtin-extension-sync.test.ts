...
describe("synced builtin extensions", () => {
	it("records the external source package versions used for vendored builtins", () => {
		...
		expect(manifest.extensions?.todowrite?.version).toBe("0.2.0");
		expect(manifest.extensions?.goal?.packageName).toBe("pi-goal");
		expect(manifest.extensions?.goal?.version).toBe("0.3.0");
	});
});
