...
async function createRuntimeWithProvider(config: ProviderConfigInput, storage = AuthStorage.inMemory()) {
	...
	await runtime.registerProvider(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID, config);
	return runtime;
}
...
describe("claude-sdk-oauth builtin provider", () => {
	...
	it("registers the provider with OAuth, catalog models and a stream fn", () => {
		...
		expect(registration.name).toBe(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID);
	...
	it("keeps claude-sdk-oauth unavailable without a stored login", async () => {
		...
		expect(runtime.hasConfiguredAuth(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID)).toBe(false);
		expect(await runtime.getAvailable(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID)).toEqual([]);
	});

	it("keeps claude-sdk-oauth unavailable with a persisted empty credential", async () => {
		const { registration } = captureRegistration();
		const storage = AuthStorage.inMemory({ [ANTHROPIC_SUBSCRIPTION_PROVIDER_ID]: emptyCredential() });
	...
	it("keeps claude-sdk-oauth available with a stored login", async () => {
		...
		expect(runtime.hasConfiguredAuth(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID)).toBe(true);
		expect(runtime.isUsingOAuth(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID)).toBe(true);
		expect(await runtime.getAvailable(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID)).not.toEqual([]);
	});
	...
	it("login selector lists the provider as oauth after registration", async () => {
		...
		expect(storage.getOAuthProviders()).toContainEqual({
			id: ANTHROPIC_SUBSCRIPTION_PROVIDER_ID,
	...
	it("preflight reaches streamSimple with a stored login", async () => {
		...
		const model = (await runtime.getAvailable(ANTHROPIC_SUBSCRIPTION_PROVIDER_ID))[0];
