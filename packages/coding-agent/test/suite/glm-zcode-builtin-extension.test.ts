...
function fetchMock(): ReturnType<typeof vi.fn> {
	return vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
		...
		if (url === "https://zcode.z.ai/api/v1/oauth/token")
			return json({ data: { zai: { access_token: "upstream-token" } } });
					...
					organizations: [
						{
							...
							isDefault: true,
							projects: [{ projectId: "project-id", isDefault: true }],
						},
					],
				},
			});
	...
	test("provisions and refreshes a Z.AI key through the registered provider", async () => {
		...
		const credentials = await oauth.login({
			...
			onManualCodeInput: async () =>
				`zcode://oauth/callback?code=auth-code&state=${new URL(authUrl).searchParams.get("state")}`,
		...
		expect(provider).toMatchObject({
			...
			config: {
				...
				models: [expect.objectContaining({ id: "glm-5.2", contextWindow: 1_000_000 })],
			},
		});
	...
	test("rejects malformed callbacks before exchange and omits response secrets", async () => {
		...
		const login = (callback: string) =>
			oauth.login({
				...
				onPrompt: async () => "",
				onManualCodeInput: async () =>
					callback.replace("{state}", new URL(authUrl).searchParams.get("state") ?? ""),
				onSelect: async () => undefined,
			});
		await expect(login("auth-code")).rejects.toThrow("callback URL");
		await expect(login("zcode://oauth:431/callback?code=auth-code&state={state}")).rejects.toThrow(
			"callback URL is invalid",
		);
		expect(fetch).not.toHaveBeenCalled();
		vi.stubGlobal(
			"fetch",
			vi.fn(() => json({ access_token: "opaque:short-secret" }, 500)),
		);
		await expect(login("zcode://oauth/callback?code=auth-code&state={state}")).rejects.not.toThrow(
			"opaque:short-secret",
		);
	});
});
