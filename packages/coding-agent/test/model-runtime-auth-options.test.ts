...
describe("ModelRuntime auth options", () => {
	...
	it("merges configured extraBody with call-site options", async () => {
		const runtime = await ModelRuntime.create({ credentials: AuthStorage.inMemory(), modelsPath: null });
		let capturedExtraBody: Record<string, unknown> | undefined;
		runtime.registerProvider("extra-body-provider", {
			baseUrl: "https://example.test/v1",
			apiKey: "generated-key",
			extraBody: { providerValue: true, shared: "provider" },
			api: "openai-completions",
			streamSimple: (_model, _context, options) => {
			...
			models: [
				{
					...testModel("extra-body-model"),
					extraBody: { modelValue: true, shared: "model" },
				},
			],
		});
		const model = runtime.getModel("extra-body-provider", "extra-body-model");
		expect(model).toBeDefined();

		await runtime.completeSimple(model!, { messages: [] }, { extraBody: { callValue: true, shared: "call" } });
	...
	it("preserves configured upstream model aliases", async () => {
		const runtime = await ModelRuntime.create({ credentials: AuthStorage.inMemory(), modelsPath: null });
		runtime.registerProvider("alias-provider", {
			baseUrl: "https://example.test/v1",
			apiKey: "generated-key",
			api: "openai-responses",
			models: [{ ...testModel("fast-alias"), upstreamModelId: "canonical-model" }],
		});
		const model = runtime.getModel("alias-provider", "fast-alias");
		expect(model).toBeDefined();
