...
describe("FileAuthStorageBackend lock-retry CPU spin", () => {
	...
	it("#given a contended auth lock #when a sync write waits through the bounded retry schedule #then user CPU stays a small fraction of wall time (no busy-wait spin)", () => {
		...
		// 3 retries at 100/200/400ms = ~700ms wall must elapse (the retry path ran),
		// but a real sleep keeps user CPU near zero while a busy-wait spin makes it ~= wall.
