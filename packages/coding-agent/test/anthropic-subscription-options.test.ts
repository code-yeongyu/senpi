...
import { resetPresetAppendDeprecation } from "../src/core/extensions/builtin/anthropic-subscription/guidance.ts";
import {
	type AnthropicSubscriptionAuthLane,
	buildAnthropicSubscriptionQueryOptions,
} from "../src/core/extensions/builtin/anthropic-subscription/options.ts";
import {
...
function optionsFor(
	providerSettings: Parameters<typeof buildAnthropicSubscriptionQueryOptions>[0]["providerSettings"],
	authLane: AnthropicSubscriptionAuthLane = "ambient",
	cwd = temporaryDirectory(),
) {
	return buildAnthropicSubscriptionQueryOptions({
		model: model(),
		context: context(),
		cwd,
		providerSettings,
...
describe("Claude SDK OAuth provider settings", () => {
	...
	it.each([
		...
		expect(loadAnthropicSubscriptionProviderSettings(manager, { [envName]: env })[key]).toEqual(expectedEnv);
		expect(loadAnthropicSubscriptionProviderSettings(manager, { [envName]: invalid })[key]).toEqual(project);
		expect(loadAnthropicSubscriptionProviderSettings(manager, {})[key]).toEqual(project);
		expect(loadAnthropicSubscriptionProviderSettings(layeredSettings({ [key]: global }), {})[key]).toEqual(global);
		expect(loadAnthropicSubscriptionProviderSettings(layeredSettings(), {})[key]).toBeUndefined();
	});

	it("tracks env as the source of an env-selected prompt mode", () => {
		const settings = loadAnthropicSubscriptionProviderSettings(layeredSettings(), {
	...
	it("accepts an empty env setting-sources list", () => {
		expect(
			loadAnthropicSubscriptionProviderSettings(layeredSettings(), { SENPI_CLAUDE_SDK_OAUTH_SETTING_SOURCES: "" }),
		).toEqual({
			settingSources: [],
		});
	});
...
describe("preset-append deprecation warning in buildAnthropicSubscriptionQueryOptions", () => {
...
describe("Claude SDK OAuth query options", () => {
	...
	it("falls back to defaults when the provider settings block is malformed", () => {
		...
		const providerSettings = loadAnthropicSubscriptionProviderSettings(SettingsManager.inMemory(malformed));
