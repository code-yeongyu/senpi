...
||||||| parent of 804ea389e9 (fix(claude-sdk-oauth): name the first diverged path on assistant_rewritten (#1975))

- An `assistant_rewritten` continuity decision now names the first diverged field path (for example `content[0].arguments.summary`) on the binding-invalidation ledger record, the `claude_sdk_oauth_session_continuity` session.log event, and the continuity observation, so the next instance of the rewrite class is a one-line diagnosis instead of a multi-week transcript hunt. Paths only — the diverged value never reaches any log. ([#1975](https://github.com/code-yeongyu/senpi/issues/1975))
