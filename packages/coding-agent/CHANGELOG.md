...
- **Self-update support for the npm scope migration**: `pi update --self` now supports the upcoming package rename from `@code-yeongyu/senpi` to `@code-yeongyu/senpi`. After the new package is published, existing global installs can update through the normal self-update flow; pi will uninstall the old global package and install the package name returned by the version check endpoint.
...
- Permission system builtin extension: full port of opencode's permission system
  ...
  - Persistent approved rules via .senpi/permissions-approved.jsonl
...
- Resource precedence for extensions, skills, prompts, themes, and slash-command name collisions is now project-first (`cwd/.pi`) before user-global (`~/.senpi/agent`). If you relied on global resources overriding project resources with the same names, rename or reorder your resources.
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@earendil-works/pi-tui`, `@earendil-works/pi-ai`, `@code-yeongyu/senpi`) are accessible to extensions without filesystem node_modules.
...
- `commands/` directories are automatically renamed to `prompts/` on startup (both `~/.senpi/agent/commands/` and `.senpi/commands/`)
...
```
# Before
~/.senpi/agent/hooks/*.ts       →  ~/.senpi/agent/extensions/*.ts
~/.senpi/agent/tools/*.ts       →  ~/.senpi/agent/extensions/*.ts
.senpi/hooks/*.ts               →  .senpi/extensions/*.ts
.senpi/tools/*.ts               →  .senpi/extensions/*.ts
```
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@code-yeongyu/senpi";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@code-yeongyu/senpi";
...
```
~/.senpi/agent/commands/*.md    →  ~/.senpi/agent/prompts/*.md
.senpi/commands/*.md            →  .senpi/prompts/*.md
```
...
- `Attachment` type removed. Use `ImageContent` from `@earendil-works/pi-ai` instead. Add images directly to message content arrays.
...
const authStorage = discoverAuthStorage(); // ~/.senpi/agent/auth.json
const modelRegistry = discoverModels(authStorage); // + ~/.senpi/agent/models.json
...
- **Symlinked skill directories**: Skills in symlinked directories (e.g., `~/.senpi/agent/skills/my-skills -> /path/to/skills`) are now correctly discovered and loaded.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@earendil-works/pi-ai`, `@earendil-works/pi-tui`, `@code-yeongyu/senpi`).
...
- Fixed custom tools failing to load from `~/.senpi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@earendil-works/pi-tui`, `@earendil-works/pi-ai`) are now resolved via aliases.
...
- Updated `@earendil-works/pi-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@earendil-works/pi-ai` with interleaved thinking support for Anthropic models.
...
- **Pi skills now use `SKILL.md` convention**: Pi skills must now be named `SKILL.md` inside a directory, matching Codex CLI format. Previously any `*.md` file was treated as a skill. Migrate by renaming `~/.senpi/agent/skills/foo.md` to `~/.senpi/agent/skills/foo/SKILL.md`.
...
- **Flexible Windows shell configuration**: The bash tool now supports multiple shell sources beyond Git Bash. Resolution order: (1) custom `shellPath` in settings.json, (2) Git Bash in standard locations, (3) any bash.exe on PATH. This enables Cygwin, MSYS2, and other bash environments. Configure with `~/.senpi/agent/settings.json`: `{"shellPath": "C:\\cygwin64\\bin\\bash.exe"}`.
...
- **File-based Slash Commands**: Create custom reusable prompts as `.txt` files in `~/.senpi/slash-commands/`. Files become `/filename` commands with first-line descriptions. Supports `{{selection}}` placeholder for referencing selected/attached content.
...
- Added thinking level persistence. Default level stored in `~/.senpi/settings.json`, restored on startup. Per-session overrides saved in session files.
...
- Add custom model configuration via `~/.senpi/models.json`
