...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --minify --keep-names --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/pi.exe"
    else
        bun build --compile --minify --keep-names --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/pi"
